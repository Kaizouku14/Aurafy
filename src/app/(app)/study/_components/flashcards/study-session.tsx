...
export const StudySession = ({ deckId, onFinish }: { deckId: string, onFinish: () => void }) => {
  ...
  // Wait. if currentIndex exceeds bounds we are done. (Handled via length check above + handleNext below)
  ...
  return (
    <div className="flex flex-col h-full w-full max-w-4xl mx-auto p-4 md:p-8 animate-in fade-in duration-300">
      ...
      <div className="flex-1 flex flex-col gap-6">
        {/* The Card Front */}
        <div className="border-4 border-border rounded-base bg-background p-8 md:p-12 shadow-sm flex flex-col justify-center min-h-[250px]">
        ...
        {!feedback ? (
          <form onSubmit={handleSubmit} className="flex-1 flex flex-col gap-4">
            <div className="flex-1 border-4 border-border rounded-base bg-secondary-background overflow-hidden flex flex-col focus-within:ring-2 focus-within:ring-main focus-within:border-main transition-all">
              <div className="bg-border px-4 py-2">
                <span className="text-xs font-bold uppercase tracking-widest text-background">Your Answer</span>
              </div>
              <textarea
                value={userAnswer}
                onChange={(e) => setUserAnswer(e.target.value)}
                placeholder="Explain the concept in your own words..."
                className="flex-1 w-full p-4 bg-transparent outline-none resize-none font-base text-lg"
                autoFocus
              />
            </div>
            ...
            <Button 
              ...
              className="w-full bg-foreground text-background font-black uppercase tracking-widest text-xl p-8 rounded-base border-4 border-transparent hover:bg-main hover:text-main-foreground transition-all sm:h-20"
            >
